...
<nav class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t-2 border-gray-200 shadow-2xl z-[9999] md:hidden safe-area-bottom mobile-bottom-nav">
    <div class="flex items-center justify-around h-14 px-1">
        <!-- Home -->
        <a href="{{ route('home') }}" class="flex flex-col items-center justify-center flex-1 py-1.5 rounded-xl transition-all {{ request()->routeIs('home') ? 'text-brand-green bg-brand-green-light' : 'text-gray-600' }} active:bg-gray-100">
        ...
        <!-- Products -->
        <a href="{{ route('products.index') }}" class="flex flex-col items-center justify-center flex-1 py-1.5 rounded-xl transition-all {{ request()->routeIs('products.*') ? 'text-brand-green bg-brand-green-light' : 'text-gray-600' }} active:bg-gray-100">
        ...
        <!-- Cart -->
        <a href="{{ route('cart.index') }}" class="flex flex-col items-center justify-center flex-1 py-1.5 rounded-xl transition-all relative {{ request()->routeIs('cart.*') ? 'text-brand-green bg-brand-green-light' : 'text-gray-600' }} active:bg-gray-100">
            <div class="relative">
                <i class="fas fa-shopping-cart text-lg mb-0.5 {{ request()->routeIs('cart.*') ? 'scale-110' : '' }} transition-transform"></i>
                <span class="absolute -top-1 -right-1 bg-gradient-to-r from-brand-green to-brand-deep-green text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center shadow-lg cart-count-mobile">0</span>
            </div>
        ...
        <!-- Account -->
        @auth
            <a href="{{ route('profile.edit') }}" class="flex flex-col items-center justify-center flex-1 py-1.5 rounded-xl transition-all {{ request()->routeIs('profile.*') || request()->routeIs('dashboard') ? 'text-brand-green bg-brand-green-light' : 'text-gray-600' }} active:bg-gray-100">
        ...
        @else
            <a href="{{ route('login') }}" class="flex flex-col items-center justify-center flex-1 py-1.5 rounded-xl transition-all {{ request()->routeIs('login') ? 'text-brand-green bg-brand-green-light' : 'text-gray-600' }} active:bg-gray-100">
