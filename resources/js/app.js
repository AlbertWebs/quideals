...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize carousel (commented out - carousel removed from home page)
    // initializeCarousel();
...
function initializeSearch() {
    const searchInput = document.querySelector('input[placeholder="Search for electronics..."]');
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            if (query.length > 2) {
                // Simulate search results for electronics
                const results = [
                    { name: 'iPhone 15 Pro', price: 'KES 189,999.00', image: 'https://via.placeholder.com/50x50/4a90e2/ffffff?text=iPhone' },
                    { name: 'Samsung Galaxy S24 Ultra', price: 'KES 175,000.00', image: 'https://via.placeholder.com/50x50/7ed321/ffffff?text=Galaxy' },
                    { name: 'MacBook Pro 14" M3', price: 'KES 450,000.00', image: 'https://via.placeholder.com/50x50/9013fe/ffffff?text=MacBook' },
                    { name: 'Sony WH-1000XM5', price: 'KES 85,000.00', image: 'https://via.placeholder.com/50x50/f5a623/ffffff?text=Sony' },
                    { name: 'PlayStation 5 Console', price: 'KES 85,000.00', image: 'https://via.placeholder.com/50x50/d0021b/ffffff?text=PS5' }
                ].filter(item => item.name.toLowerCase().includes(query.toLowerCase()));
                
                if (results.length > 0) {
                    searchResults.innerHTML = results.map(item => `
                        <div class="flex items-center p-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100">
                            <img src="${item.image}" alt="${item.name}" class="w-10 h-10 rounded mr-3">
                            <div>
                    ...
                    searchResults.innerHTML = '<div class="p-4 text-gray-500">No electronics found</div>';
                }
...
function initializeMobileFilters() {
    ...
    function openFilters() {
        ...
        // Use requestAnimationFrame to ensure the element is visible before animating
        requestAnimationFrame(function() {
            const filterPanel = mobileFilters.querySelector('.fixed.bottom-0');
            if (filterPanel) {
    ...
    // Close filters - backdrop click
    if (mobileFilterBackdrop) {
        mobileFilterBackdrop.addEventListener('click', function(e) {
            e.preventDefault();
            closeFilters();
        });
    }
    
    // Apply filters button - closes the drawer
    if (mobileApplyFilters) {
        mobileApplyFilters.addEventListener('click', function(e) {
            e.preventDefault();
            // The filters are already applied via links, just close the drawer
            closeFilters();
        });
    }
...
function initializeProductImageSwitching() {
    ...
    if (!thumbnails.length || !mainImage) {
        console.log('Product image switching not needed on this page');
        return;
    }
    
    console.log('Initializing product image switching with', thumbnails.length, 'thumbnails');
    
    // Add click event listeners to thumbnails
    thumbnails.forEach((thumbnail, index) => {
        console.log(`Setting up thumbnail ${index + 1}:`, thumbnail);
        ...
        thumbnail.addEventListener('click', function(e) {
            ...
            console.log('Thumbnail clicked, image src:', imageSrc);
        ...
        thumbnail.addEventListener('touchstart', function(e) {
            e.preventDefault();
            const imageSrc = this.getAttribute('data-image');
            console.log('Thumbnail touched, image src:', imageSrc);
    ...
    window.changeMainImage = function(thumbnail, imageSrc) {
        console.log('Changing main image to:', imageSrc);
        ...
        newImage.onload = function() {
            ...
            // Add a brief flash effect to show the change
    ...
    // Add scroll functionality for thumbnail gallery
    ...
    window.testImageSwitch = function() {
        ...
        if (thumbnails.length > 1) {
            console.log('Testing image switch with second thumbnail');
            const secondThumbnail = thumbnails[1];
            const imageSrc = secondThumbnail.getAttribute('data-image');
            changeMainImage(secondThumbnail, imageSrc);
        } else {
            console.log('Not enough thumbnails to test');
        }
    };
}
